...
void UpdateAngleForces(System& sys){
    Angle& angle = sys.molecules.angles[0];
...
/* nonbonded forces: only a force between atoms in different molecules
    ...
    for (auto& atom1 : sys.molecules.atoms){ 
        ...
        #pragma omp simd reduction(+:accumulated_forces_non_bond) collapse(2)
...
System MakeWater(int N_molecules){
    ...
    Atoms Oatom(16, 0.65,    0.31, -0.82, "O",N_molecules);  // Oxygen atoms
    Atoms Hatom1( 1, 0.18828, 0.238, 0.41, "H",N_molecules); // Hydrogen atoms
